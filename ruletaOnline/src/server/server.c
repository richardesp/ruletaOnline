                        ...
                        if (recibidos > 0)
                        {
                            ...
                            else if (strncmp(buffer, "USUARIO ", 8) == 0)
                            {
                                ...
                                if (comprobarUsername(user))
                                {
                                    ...
                                    strncpy(user, arrayClientes[index].nickName, MSG_SIZE);

                                    printf("%s\n", arrayClientes[index].nickName);

                                    bzero(buffer, sizeof(buffer));
                                    strcpy(buffer, "Usuario introducido registrado en el sistema\n");
                                ...
                                {
                                    bzero(buffer, sizeof(buffer));
                                    strcpy(buffer, "Usuario introducido no registrado en el sistema. Puede registrarse con la sentencia \'Registrar\'\n");
                                    if (send(i, buffer, sizeof(buffer), 0) == -1)
                                    {
                                        perror("Error al invocar a send().");
                                    }
                            ...
                            else if (strncmp(buffer, "PASSWORD ", 9) == 0)
                            {
                                ...
                                if(comprobarPassword(password, arrayClientes[index].nickName)) {
                                    ...
                                    strcpy(password, arrayClientes[index].password);

                                    bzero(buffer, sizeof(buffer));
                                    strcpy(buffer, "Contraseña introducida correcta\n");
                                    if (send(i, buffer, sizeof(buffer), 0) == -1)
                                    {
                                        perror("Error al invocar a send().");
                                    }
                                ...
                                else
                                {
                                    ...
                                    bzero(buffer, sizeof(buffer));
                                    strcpy(buffer, "Contraseña introducida incorrecta\n");
                                    if (send(i, buffer, sizeof(buffer), 0) == -1)
                                    {
                                        perror("Error al invocar a send().");
                                    }
...
bool comprobarPassword(char *passwordToCompare, char *usernameToCompare) {
    ...
    while (fgets(buffer, 256, file))
    {
        ...
        printf("%s == %s || %s == %s\n", username, usernameToCompare, password, passwordToCompare);
